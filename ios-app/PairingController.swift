...
final class PairingController: ObservableObject {
    ...
    enum PairingError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .busy: return "配對已在進行中。"
            case .localNetworkDenied: return "「區域網路」權限已關閉。請到設定 › AirCard-iOS › 區域網路 開啟。"
            case .zeroBytes: return "配對產生了空白檔案。請核准配對要求後再試一次。"
    ...
    private func finish(_ outcome: Outcome) {
        ...
        case let .success(name, path):
            ...
            if size == 0 {
                pairingStatus = "失敗：空白配對檔"
                resolve(.failure(PairingError.zeroBytes))
            } else {
                pairingStatus = "已配對：\(name)（\(size)B）"
                resolve(.success(canonical))
            }
        case let .failure(message):
            pairingStatus = "失敗：\(message)"
    ...
    fileprivate func startAdvertising(serviceID: String, port: Int32, txt: [String: Data]) {
        ...
        service.publish()
        netService = service
        pairingStatus = "廣播中 — 請開啟設定 › 隱私權與安全性 › 開發人員模式"
    }

    fileprivate func presentPin(_ pin: String) {
        pairingPIN = pin
        pairingStatus = "在設定 › 隱私權與安全性 › 開發人員模式 › Pair with AirCard-iOS 輸入 PIN \(pin)"
    }
