    ...
    public func importTendie(from sourceURL: URL) async throws -> TendieItem {
        ...
        guard FileManager.default.fileExists(atPath: destinationURL.path) else {
            throw NSError(
                ...
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: "無法將桌布檔案儲存到 \(destinationURL.path)"]
            )
        }
        ...
        guard extractRC == 0 else {
            throw NSError(
                domain: "TendiesEngine",
                code: Int(extractRC),
                userInfo: [NSLocalizedDescriptionKey: "無法解壓縮 .tendies zip 封存檔（代碼 \(extractRC)）"]
            )
        }
    ...
    public func flashTendies(
        ...
        if normalizedContainer.isEmpty {
            throw NSError(
                domain: "TendiesEngine",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "需要 PosterBoard 容器路徑。"]
            )
        }
        ...
        for dir in dirsToWrite {
            ...
            if !writeOk {
                throw NSError(
                    domain: "TendiesEngine",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "無法寫入目錄：\(errDesc ?? "exploit error")"]
                )
            }
        }
    }
    ...
    private func injectDescriptorFolder(
        ...
        if !ok {
            throw NSError(
                domain: "TendiesEngine",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "無法注入 descriptor：\(errDesc ?? "exploit error")"]
            )
        }
    }
