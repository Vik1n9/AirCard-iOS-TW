...
final class AppViewModel: ObservableObject {
    ...
    func importPairingFile(from sourceURL: URL, originalName: String? = nil) -> Bool {
        ...
        do {
            ...
            refreshPairingFile()
            pairingStatus = "配對檔已載入 ✅ (\(originalName ?? "aircard_pairing.plist"))"
            return true
        } catch {
            errorMessage = "無法儲存配對檔：\(error.localizedDescription)"
            return false
        }
    }
    ...
    func startPairing() {
        ...
        Task {
            do {
                ...
                self.pairingPhase = .idle
                self.pairingStatus = ""
                self.errorMessage = "配對失敗：\(error.localizedDescription)"
            }
        }
    ...
    func startCardScanning() {
        ...
        guard hasPairingFile else {
            errorMessage = "掃描前需要配對檔。請先配對此 iPhone，或選取一個 .plist。"
            return
        }
        ...
        withTransaction(t) {
            ...
            scanStatusText = "開啟 Apple Pay（連按兩下 Side button）並輕點你的卡片…"
        }
        log.append("已啟動即時卡片掃描…")
        ...
        let thread = Thread {
            ...
            DispatchQueue.main.async {
                ...
                if rc != 0 {
                    ...
                    vm.scanStatusText = "掃描已停止：\(msg)"
                    vm.log.append("❌ 掃描錯誤：\(msg)")
                    vm.errorMessage = "卡片掃描錯誤：\(msg)"
                } else {
                    vm.scanStatusText = "掃描已停止。卡片總數：\(vm.cards.count)。"
                    vm.log.append("掃描已停止。卡片總數：\(vm.cards.count)。")
                }
            }
        }
    ...
    func stopCardScanning() {
        ...
        withTransaction(t) {
            ...
            scanStatusText = "掃描已停止。卡片總數：\(cards.count)。"
        }
        saveCards()
    }
            ...
            for m in matches {
                if m.numberOfRanges > 1, let r = Range(m.range(at: 1), in: line) {
                    ...
                    if !self.cards.contains(where: { $0.id == candidate }) {
                        ...
                        self.saveCards()
                        self.scanStatusText = "找到卡片：\(candidate)"
                        self.log.append("找到卡片：\(candidate)")
    ...
    func flashCards() {
        ...
        if !vpnUp {
            cardFlashLog.append("⚠️ 注意：未偵測到 Loopback VPN，改以直接 loopback（127.0.0.1）嘗試...")
        }
        ...
        Task.detached { [weak self] in
            ...
            for (i, card) in selected.enumerated() {
                ...
                await MainActor.run {
                    self.cardFlashLog.append("[\(i+1)/\(selected.count)] 正在刷寫卡片 \(cleanId.prefix(12))…")
                ...
                guard let sourceImg = sourceImg else {
                    await MainActor.run { self.cardFlashLog.append("  ⚠️ 卡片 \(cleanId.prefix(8)) 沒有圖片") }
                    continue
                }
                ...
                guard !allSkins.isEmpty else {
                    await MainActor.run { self.cardFlashLog.append("  ⚠️ 無法產生卡片外觀") }
                    continue
                }
                ...
                await MainActor.run {
                    self.cardFlashLog.append("  ⚡ 正在將外觀注入 \(cleanId.prefix(10)).pkpass…")
                }
                ...
                if !writeOk {
                    await MainActor.run {
                        self.cardFlashLog.append("  ❌ 無法寫入卡片外觀：\(errDesc ?? "exploit error")")
                    }
                    continue
                }

                await MainActor.run {
                    self.cardFlashLog.append("  ✅ 外觀已套用！正在使通行卡快取失效…")
                }
                ...
                successCount += 1
                await MainActor.run {
                    self.cardFlashLog.append("  ✅ 通行卡快取已失效")
            ...
            await MainActor.run {
                if successCount > 0 {
                    ...
                    self.cardFlashLog.append("🎉 已刷寫 \(successCount)/\(selected.count) 張卡片！請強制關閉 Wallet App 以查看變更。")
                    self.successAlertMessage = "已成功將外觀套用至 \(successCount) 張卡片！\n\n請在 iPhone 上強制關閉 Wallet App（或重新開機）以查看新設計。"
                    self.showSuccessAlert = true
                } else {
                    self.cardFlashPhase = .done(ok: false)
                    self.cardFlashLog.append("❌ 卡片刷寫失敗。請檢查連線後再試一次。")
                }
            }
        }
    }
        ...
        Task.detached {
            ...
            await MainActor.run {
                if let (keys, rawData, count) = result {
                    ...
                    self.errorMessage = "無法讀取 .passthm — 格式無效或不支援。"
                }
            }
        }
    }
    ...
    func flashPassthm() {
        ...
        if !vpnUp {
            passthmFlashLog.append("⚠️ 注意：未偵測到 Loopback VPN，改以直接 loopback（127.0.0.1）嘗試...")
        }
        ...
        Task.detached { [weak self] in
            ...
            await MainActor.run {
                self.passthmFlashLog.append("⚡ 已暫存主題資源 (\(targetVer) · \(langs.joined(separator: ", ").uppercased()) · \(targetBold.code))。正在注入到 iOS 快取…")
            ...
            for (idx, targetPath) in targetDirs.enumerated() {
                ...
                await MainActor.run {
                    self.passthmFlashLog.append("  正在寫入 \(targetName)…")
                }
                ...
                if !stepOk {
                    allOk = false
                    await MainActor.run {
                        self.passthmFlashLog.append("  ⚠️ 寫入 \(targetName) 失敗：\(lastErr ?? "error")")
                    }
                } else {
                    await MainActor.run {
                        self.passthmFlashLog.append("  ✅ 已注入到 \(targetName)")
                    }
                }
            ...
            await MainActor.run {
                if allOk {
                    ...
                    self.passthmFlashLog.append("🎉 密碼主題已套用！請鎖定 iPhone 以查看效果。")
                    self.successAlertMessage = "密碼主題已成功套用！\n\n請鎖定 iPhone（或重新開機）以查看新的密碼鍵盤。"
                    self.showSuccessAlert = true
                } else {
                    self.passthmFlashPhase = .done(ok: false)
                    self.passthmFlashLog.append("❌ 一或多個主題注入失敗。")
                }
            }
        }
    }
    ...
    func exportPassthm() -> URL? {
        ...
        do {
            ...
            errorMessage = "無法匯出主題：\(error.localizedDescription)"
            return nil
        }
    }
    ...
    func importTendieFiles(urls: [URL]) async {
        ...
        for url in urls {
            do {
                ...
                await MainActor.run {
                    self.errorMessage = "無法匯入 \(url.lastPathComponent)：\(error.localizedDescription)"
                }
            }
        }
    }
    ...
    func autoDetectPosterBoardContainer(silent: Bool = false) async {
        ...
        guard FileManager.default.fileExists(atPath: pairingPath) else {
            if !silent {
                await MainActor.run {
                    self.errorMessage = "目前沒有作用中的配對檔。請先到「配對」分頁配對裝置。"
                }
            }
            return
        }
        ...
        do {
            ...
            if !silent {
                await MainActor.run {
                    self.errorMessage = "自動偵測失敗：\(error.localizedDescription)\n請確認 LocalDevVPN 已連線且裝置已解鎖。"
                }
            }
        }
    }
    ...
    func flashSelectedTendies() async {
        ...
        guard !selected.isEmpty else {
            errorMessage = "尚未選取要刷寫的桌布。"
            return
        }

        let pairingPath = PairingController.pairingFilePath()
        guard FileManager.default.fileExists(atPath: pairingPath) else {
            errorMessage = "目前沒有作用中的配對檔。請先配對裝置。"
            return
        }
        ...
        if container.isEmpty {
            do {
                ...
                errorMessage = "無法自動找到 PosterBoard 容器。請確認 LocalDevVPN 已連線且 iPhone 已解鎖。"
                return
            }
        }
        ...
        do {
            ...
            tendiesFlashLog.append("🎉 桌布已成功套用！")
            tendiesFlashLog.append("⚡ 正在觸發 NeoSpring 重新載入 SpringBoard...")
            ...
            tendiesFlashLog.append("❌ 錯誤：\(error.localizedDescription)")
