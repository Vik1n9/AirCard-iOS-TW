            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text("適用於 iOS 18+ 的 Apple Wallet 卡片外觀與密碼主題")
                            .font(.caption)
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                Label("主導與核心開發者", systemImage: "crown.fill")
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                Label("基礎 IPA 開發者", systemImage: "hammer.fill")
                                    .font(.caption.bold().uppercaseSmallCaps())
                                    .foregroundStyle(.blue)
                                Spacer()
                                Text("基礎")
                            ...
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    Text("NeoSpring & PosterBoard")
                                        .font(.subheadline.bold())
                                    Text("SpringBoard 重新載入與 .tendies 桌布（@neonmodder123、@skadz108、@rooootdev）")
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
                            }
...
struct ContentView: View {
    ...
    var body: some View {
        TabView(selection: $vm.selectedTab) {
            PairingTab()
                .tabItem { Label("配對", systemImage: "antenna.radiowaves.left.and.right") }
                .tag(AppTab.pairing)

            WalletCardsTab()
                .tabItem { Label("Wallet 卡片", systemImage: "creditcard.fill") }
                .tag(AppTab.walletCards)

            PasscodeThemeTab()
                .tabItem { Label("密碼", systemImage: "lock.circle.fill") }
                .tag(AppTab.passcodeThemes)

            TendiesView()
                .tabItem { Label("桌布", systemImage: "photo.stack.fill") }
                .tag(AppTab.wallpapers)
        }
        .alert("注意", isPresented: Binding(
            ...
            Form {
                ...
                Section {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text("透過 AirTraffic 沙盒逃逸，在裝置上套用自訂 Wallet 卡片外觀與密碼主題。")
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                    }
                    .padding(.vertical, 4)
                ...
                .confirmationDialog(
                    ...
                    Button("刪除", role: .destructive) { vm.deletePairingFile() }
                ...
                Section("在此 iPhone 上配對") {
                    if vm.pairingPhase == .pairing {
                        VStack(alignment: .leading, spacing: 12) {
                            HStack(spacing: 8) {
                                ProgressView().scaleEffect(0.85)
                                Text(vm.pairingStatus.isEmpty ? "正在啟動本機配對主機…" : vm.pairingStatus)
                                    .font(.subheadline)
                                    .foregroundStyle(.secondary)
                            }
                            ...
                            if let pin = vm.pairingPIN {
                                VStack(alignment: .leading, spacing: 12) {
                                    ...
                                    Text("設定 › 隱私權與安全性 › 開發人員模式 › Pair with AirCard-iOS")
                                     ...
                                     Button {
                                        ...
                                        Label("立即開啟「設定」App", systemImage: "arrow.up.forward.app")
                                            .bold()
                        ...
                        VStack(spacing: 12) {
                            ...
                            Button {
                                ...
                                HStack(spacing: 8) {
                                    ...
                                    Text(vm.hasPairingFile ? "重新配對此 iPhone" : "配對此 iPhone")
                                        .font(.headline)
                                    Spacer()
                                }
                                .frame(maxWidth: .infinity)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(vm.vpnUp ? "Loopback VPN 作用中" : "未偵測到 Loopback VPN")
                        .font(.subheadline.bold())
                    Text(vm.vpnUp
                         ? "RSD 通道就緒 — exploit 即將連線。"
                         : "執行刷寫前請先連線 LocalDevVPN。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            if !vm.vpnUp {
                VStack(alignment: .leading, spacing: 6) {
                    Text("設定 LocalDevVPN：")
                        .font(.caption.bold())
                    ForEach([
                        "1. 開啟 LocalDevVPN App 並輕點連線。",
                        "2. 返回 AirCard-iOS — 狀態指示會變成綠色。"
                    ], id: \.self) { step in
                        Text(step)
                            .font(.caption)
                            .foregroundStyle(.secondary)
...
struct WalletCardsTab: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Wallet 卡片（\(vm.cards.count)）")
            .toolbar {
                    ...
                    Menu {
                        ...
                        if !vm.cards.isEmpty {
                            Button {
                                ...
                                Label("為所有卡片設定外觀...", systemImage: "photo.on.rectangle.angled")
                            }
                            ...
                            Button(role: .destructive) {
                                ...
                                Label("清除所有卡片", systemImage: "trash")
                            }
                            ...
                            Button {
                                ...
                                Label("致謝", systemImage: "heart.fill")
                            }
                        }
                    } label: {
            ...
            .confirmationDialog("選擇圖片來源", isPresented: $showSourceDialog, titleVisibility: .visible) {
                Button {
                    isPhotosPickerPresented = true
                } label: {
                    Label("照片圖庫", systemImage: "photo.on.rectangle")
                }
                Button {
                    isDocumentPickerPresented = true
                } label: {
                    Label("從「檔案」選擇…", systemImage: "folder")
                }
                Button("取消", role: .cancel) {
                    activePicker = nil
                }
    ...
    private var cardsList: some View {
        VStack(spacing: 16) {
            ...
            if !vm.cardFlashLog.isEmpty {
                CompactLogView(
                    title: "刷寫紀錄（\(vm.cardFlashLog.count) 行）",
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(alignment: .top, spacing: 10) {
                    Text("2.")
                        .bold()
                        .foregroundStyle(.blue)
                    Text("在此 iPhone 上，**連按兩下 Side button**（Apple Pay），以 **Face ID** 驗證，然後**輕點你的卡片**。")
                }
                HStack(alignment: .top, spacing: 10) {
                    Text("3.")
                        .bold()
...
struct AddCardSheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("卡片雜湊") {
                    TextField("貼上卡片雜湊（例如 M6nDwZrkYbFl…）", text: $hashText, axis: .vertical)
        ...
        NavigationStack {
            Form {
                ...
                if !vm.passthmFlashLog.isEmpty {
                    Section {
                        CompactLogView(
                            title: "刷寫紀錄（\(vm.passthmFlashLog.count) 行）",
...
struct ApplyThemeSection: View {
    ...
    var body: some View {
        ...
        if !vm.documentsThemes.isEmpty {
            Section("App 資料夾中的主題（我的 iPhone › AirCard-iOS）") {
        ...
        Section("瀏覽檔案") {
            HStack {
                Button {
                    ...
                    Label(vm.loadedTheme == nil ? "從「檔案」選擇 .passthm…" : "更換 .passthm…",
        ...
        if let theme = vm.loadedTheme {
            ...
            Section("主題資訊") {
                LabeledContent("主題中的檔案", value: "\(theme.fileCount)")
                LabeledContent("已設定數字樣式", value: "\(theme.keysPreview.count) 個按鍵")

                Button {
                    vm.adoptThemeIntoCreator()
                } label: {
                    Label("在主題建立器中編輯", systemImage: "pencil")
                        .frame(maxWidth: .infinity, alignment: .center)
                }
                .buttonStyle(.bordered)
            }
        ...
        if case .running = vm.passthmFlashPhase {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("正在刷寫主題…").font(.subheadline.bold())
        ...
        Section {
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Picker("系統快取", selection: $vm.targetTelephonyVersion) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("系統語言")
                        .font(.caption.bold())
                        .foregroundColor(.secondary)
                    Picker("系統語言", selection: $vm.passcodeLanguageTarget) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("字重／樣式")
                        .font(.caption.bold())
                        .foregroundColor(.secondary)
                    Picker("字重／樣式", selection: $vm.passcodeBoldTarget) {
                ...
                HStack(alignment: .top, spacing: 6) {
                    ...
                    if vm.passcodeLanguageTarget == .all && vm.passcodeBoldTarget == .both {
                        Text("通用模式會刷寫約 600 個檔案，涵蓋所有語言與粗體文字。選取特定語言（例如烏克蘭文）可大幅加快刷寫速度。")
                            .font(.caption2)
                            .foregroundColor(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    } else {
                        Text("已選取快速模式：僅針對 \(vm.passcodeLanguageTarget.rawValue) 與 \(vm.passcodeBoldTarget.rawValue)。")
                            .font(.caption2)
...
struct ThemeCreatorSection: View {
        ...
        Group {
            Section("海報圖片") {
                Button {
                    ...
                    Label(vm.posterImage == nil ? "為鍵盤選擇照片…" : "更換照片…",
                          systemImage: "photo")
                        .frame(maxWidth: .infinity, alignment: .center)
                }
            }
            .confirmationDialog("選擇海報圖片來源", isPresented: $showPosterSourceDialog, titleVisibility: .visible) {
                Button {
            ...
            if vm.posterImage != nil {
                Section("裁切樣式") {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text(vm.maskToCircles ? "圖稿會裁切成個別的圓形按鈕圖示。" : "無縫圖稿會橫跨撥號鍵，不做圓形裁切（Adobe Dog 風格）。")
                            .font(.caption2)
                            .foregroundColor(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
    ...
    private var individualKeysSection: some View {
        ...
        .confirmationDialog("選擇按鍵 \(selectedDigitForPicker ?? "") 的圖片來源", isPresented: $showKeySourceDialog, titleVisibility: .visible) {
            Button {
